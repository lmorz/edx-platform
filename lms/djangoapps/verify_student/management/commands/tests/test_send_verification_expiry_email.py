...
class TestSendVerificationExpiryEmail(MockS3BotoMixin, TestCase):
    ...
    def create_expired_software_secure_photo_verification(self):
        ...
        verification.expiration_datetime = now() - timedelta(days=self.days)
        verification.save()
        return verification

    def test_expiration_datetime_range(self):
        """
        ...
        verification_in_range.expiration_datetime = now() - timedelta(days=self.days)
        verification_in_range.save()

        user = UserFactory.create()
        verification = self.create_and_submit(user)
        verification.status = 'approved'
        verification.expiration_datetime = now() - timedelta(days=self.days + 1)
        verification.save()
    ...
    def test_expiry_email_date_range(self):
        ...
        verification_in_range = self.create_and_submit(user)
        verification_in_range.status = 'approved'
        verification_in_range.expiration_datetime = today - timedelta(days=self.days + 1)
        verification_in_range.expiry_email_date = today - timedelta(days=self.resend_days)
        verification_in_range.save()

        call_command('send_verification_expiry_email')

        # Check that email is sent even if the verification is not in expiration_datetime range but matches
    ...
    def test_not_enrolled_in_verified_course(self):
        ...
        verification = self.create_and_submit(user)
        verification.status = 'approved'
        verification.expiration_datetime = now() - timedelta(days=self.resend_days * (self.default_no_of_emails - 1))
    ...
    def test_number_of_emails_sent(self):
        ...
        verification.expiration_datetime = now() - timedelta(days=1)
        verification.save()
        call_command('send_verification_expiry_email')
        ...
        for i in range(1, self.default_no_of_emails + 1):
            if SoftwareSecurePhotoVerification.objects.get(pk=verification.id).expiry_email_date:
                today = now().replace(hour=0, minute=0, second=0, microsecond=0)
                verification.expiration_datetime = today - timedelta(days=self.resend_days * i + 1)
                verification.expiry_email_date = today - timedelta(days=self.resend_days)
                verification.save()
                call_command('send_verification_expiry_email')
            else:
